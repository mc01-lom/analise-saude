...
df["custo_norm"] = df.apply(
    lambda r: r["custo_eq40"] * FATOR_PJ if r["reg"] == "PJ" else r["custo_eq40"],
    axis=1)
df["rh_norm"] = df.apply(
...
for idx, (color, titulo, linhas) in enumerate(blocos):
    ...
    for j, linha in enumerate(linhas):
        ax1.text(0.03, y0-0.50-j*0.038, linha,
...
for _, r in exemplos.iterrows():
    rows.append([r["cargo"], r["uf"], r["forn"], r["reg"],
                 f"{int(r['jh'])}h/sem",
                 brl(r["custo"]),
                 f"R${r['custo_hora']:.0f}/h",
                 f"R${r['rh_norm']:.0f}/h"])
col_labels = ["Cargo","UF","Forn.","Reg.","Jornada",
              "Custo\nnominal","Custo/h\nbruto","Custo/h\nnorm."]
...
ax2.text(0.5, 0.0,
         "Col. destacada (Custo/h norm.) = valor comparável entre todos os postos",
         transform=ax2.transAxes, ha="center", fontsize=7.5,
         color="#566573", style="italic")

plt.tight_layout()
plt.savefig("g1_0_metodologia.png", dpi=150, bbox_inches="tight")
plt.close()
...
inv = (df.groupby(["forn","cargo","reg"])
         ...
         .reset_index()
         .sort_values(["forn","N"], ascending=[True,False]))
...
for ax, (forn, color) in zip(axes, [("Porto",C_PORTO),("3778",C_3778)]):
    ...
    labels = [f"{r.cargo}\n({r.reg} · {int(r.jornada_med)}h)"
              for _, r in sub.iterrows()]
    ax.barh(range(len(sub)), sub["N"], color=color,
            height=0.55, edgecolor=WH, zorder=3)
    for i, (_, row) in enumerate(sub.iterrows()):
        ax.text(row["N"]+0.1, i, str(int(row["N"])),
...
plt.tight_layout()
plt.savefig("g1_1_inventario.png", dpi=150, bbox_inches="tight")
plt.close()
...
for ax, (col, titulo) in zip(axes, [
    ...
    for i, uf in enumerate(ufs):
        ...
        if len(vp):
            ax.bar(i-w/2, vp.median(), width=w, color=C_PORTO,
                   edgecolor=WH, zorder=3,
                   label="Porto Seguro" if i==0 else "")
            ax.text(i-w/2, vp.median()+2, f"R${vp.median():.0f}",
                    ha="center", va="bottom", fontsize=7.5,
                    fontweight="bold", color=C_PORTO)
        if len(v3):
            ax.bar(i+w/2, v3.median(), width=w, color=C_3778,
                   edgecolor=WH, zorder=3,
                   label="3778 Gestiona" if i==0 else "")
            ax.text(i+w/2, v3.median()+2, f"R${v3.median():.0f}",
                    ha="center", va="bottom", fontsize=7.5,
                    fontweight="bold", color=C_3778)
...
plt.tight_layout()
plt.savefig("g1_2_medico_hora_uf.png", dpi=150, bbox_inches="tight")
plt.close()
print("✅ g1_2_medico_hora_uf.png")


# ════════════════════════════════════════════════════════════════════════════
# 1.3 DISTRIBUIÇÃO GEOGRÁFICA — BARRAS HORIZONTAIS
# ════════════════════════════════════════════════════════════════════════════
...
for ax, (forn, label, color) in zip(axes, [
    ("Porto","Porto Seguro",C_PORTO),
    ("3778", "3778 Gestiona",C_3778),
]):
    ...
    ax.set_yticks(range(len(sub)))
    ax.set_yticklabels(sub["uf"], fontsize=10)
...
plt.tight_layout()
plt.savefig("g1_3_barras_uf.png", dpi=150, bbox_inches="tight")
plt.close()
...
plt.tight_layout()
plt.savefig("g1_4_boxplot.png", dpi=150, bbox_inches="tight")
plt.close()
...
plt.tight_layout()
plt.savefig("g1_7_heatmap.png", dpi=150, bbox_inches="tight")
plt.close()
print("✅ g1_7_heatmap.png")

print("\nQ1 concluída — 6 gráficos gerados.")
